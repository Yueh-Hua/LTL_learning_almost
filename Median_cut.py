...
def lum(a,b,c):
    ...
    return a*0.2125 + b*0.7154 + c*0.0721 #可以改變 
    ...
    for i in label_nolabel:
        ...
        if count_box[cnt].type == "Area":
            ...
            for v in count_box[cnt].accum[3]:
                ...
                tar["color"] = color[:,v[0],v[1]].tolist() # rgb 現在這裡是存 hdr 的rgb 0~1 如果想要用 LDR 就改成 origin_image 0~255
                tar["intensity"] = v[2]
                light.append(tar)
                pass
        else:
            ...
            tar["color"] = color[:,count_box[cnt].bright[0],count_box[cnt].bright[1]].tolist()# rgb
